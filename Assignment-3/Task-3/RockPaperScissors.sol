...
contract RockPaperScissors {
    ...
    function resolveGame(uint256 gameId) internal {
        ...
        // Distribute rewards based on the result using `call`
        uint256 reward = betAmount * 2;  // Total reward amount

        if (result == GameResult.Player1Win) {
            (bool success, ) = game.player1.call{ value: reward }("");
            require(success, "Transfer to Player 1 failed.");
        } else if (result == GameResult.Player2Win) {
            (bool success, ) = game.player2.call{ value: reward }("");
            require(success, "Transfer to Player 2 failed.");
        } else if (result == GameResult.Draw) {
            (bool success1, ) = game.player1.call{ value: betAmount }("");
            (bool success2, ) = game.player2.call{ value: betAmount }("");
            require(success1 && success2, "Refund to players failed.");
        }
